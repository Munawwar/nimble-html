...
if (!mode || !['prepare', 'package', 'install', 'dev'].includes(mode)) {
	console.error('Usage: node tooling/nimble-html-vscode/scripts/local-dev.js <prepare|package|install|dev>')
	process.exit(1)
}
...
if (mode === 'prepare' || mode === 'package' || mode === 'install') {
	...
	for (const packageName of ['nimble-html-typescript-plugin', 'nimble-html-template-analyzer']) {
