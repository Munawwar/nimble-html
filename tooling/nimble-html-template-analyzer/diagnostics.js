...
function createDiagnostics(context) {
	...
	function getTemplateCompletions(position) {
		...
		function buildEntries(namespace, tagName, partial, forcePrefix, quoted) {
			...
			/**
			 * Add filtered completion entries with a shared prefix/sort bucket.
			 * @param {string[]} names
			 * @param {string} prefix
			 * @param {string} sortText
			 * @param {boolean} [shouldQuote]
			 */
		...
		for (let index = 0; index < node.template.templateSpans.length; index++) {
			...
			// Spread object keys follow the same completion rules as inline attributes.
